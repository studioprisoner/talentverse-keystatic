            ...
            <div className="py-6 mx-10">
                ...
                <div className="mt-10">
                    <div className="grid grid-flow-col grid-rows-2 gap-y-8 gap-x-24 sm:grid-rows-1" aria-label="Tabs">
                        {tabs.map((tab) => (
                            <a href={tab.href} key={tab.name} className="flex items-center justify-center px-6 py-2 text-base font-medium text-white uppercase rounded-full sm:text-lg bg-gun-powder-700">
                                {tab.name}
                            </a>
                        ))}
                    </div>
